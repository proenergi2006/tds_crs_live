<template>
        ...
        <div class="flex items-center mb-4 justify-between">
          <div>
            ...
            <h1 class="text-2xl font-semibold mt-2">
              Verifikasi Penawaran Proenergi {{ penawaran.nomor_penawaran }}
            </h1>
          </div>
        </div>
  ...
  <div class="bg-white p-6 rounded shadow text-sm mb-6 border border-slate-200">
    ...
    <div class="grid grid-cols-2 gap-y-2 gap-x-6">
      ...
      <div class="grid grid-cols-3">
        ...
        <span class="col-span-2">
          : {{ formatCurrency(penawaran.produk_harga?.harga_price_list_pe) }}
        </span>
      </div>
      <div class="grid grid-cols-3">
  ...
  async function verifikasi() {
    ...
    await axios.patch(`/api/penawarans-proenergi/${id}/verifikasi`, { catatan: result.value })
  ...
  async function tolak() {
    ...
    if (!result.isConfirmed) return
    await axios.patch(`/api/penawarans-proenergi/${id}/tolak-bm`, { catatan: result.value })
