...
/* Proenergi: sampai_dengan auto-fill ke akhir bulan dari masa_berlaku */
watch(() => form.masa_berlaku, (val) => {
  if (!val) { form.sampai_dengan = ''; return }
  const date = new Date(val)
  if (isNaN(date.getTime())) { form.sampai_dengan = ''; return }
  ...
  form.sampai_dengan = `${year}-${month}-${day}`
})
...
async function fetchPenawaran() {
  try {
    const { data } = await axios.get(`/api/penawarans-proenergi/${idParam}`)
...
async function checkHarga(item: ItemLine) {
  ...
  try {
    ...
    if (data.found) {
      /* Proenergi menggunakan harga_price_list_pe sebagai harga acuan */
      item.harga_price_list = Number(data.harga_price_list_pe ?? data.harga_price_list ?? 0)
...
function validateForm(): boolean {
  ...
  if (form.tipe_pembayaran === 'TOP' && !form.top_hari) {
    msgs.push('TOP hari wajib dipilih jika tipe pembayaran TOP.')
  }
...
async function submitForm() {
  ...
  try {
    ...
    if (isEdit) {
      await axios.put(`/api/penawarans-proenergi/${idParam}`, payload)
      success('Berhasil', 'Penawaran berhasil diupdate.')
    } else {
      await axios.post('/api/penawarans-proenergi', payload)
  ...
  <div class="p-6 intro-y">
    <div class="flex items-center mb-4 gap-3">
      <div class="flex flex-col gap-2">
        <h2 class="text-lg font-medium">
          {{ isEdit ? 'Edit Penawaran Proenergi' : 'Tambah Penawaran Proenergi' }}
        </h2>
    ...
    <div class="bg-white shadow rounded-lg p-6">
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <!-- Masa Berlaku — sampai_dengan readonly, auto-fill akhir bulan -->
        <div class="grid grid-cols-2 gap-4">
          <div>
          ...
          <div>
            ...
            <input v-model="form.sampai_dengan" type="date" class="w-full border rounded p-2 bg-gray-100"
              :class="inputClass('sampai_dengan')" readonly />
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
          <!-- Tipe Pembayaran — Proenergi memakai TOP + top_hari selector -->
          <div class="space-y-2">
            ...
            <transition name="fade">
              <div v-if="form.tipe_pembayaran === 'TOP'" class="mt-3">
                <label class="block text-sm font-medium text-slate-700 mb-1">TOP (Hari)</label>
                <FormSelect v-model="form.top_hari" class="w-full">
          ...
          <!-- Acuan Pembayaran — hanya Proenergi -->
          <div class="space-y-2">
            <label class="block text-sm font-medium text-gray-700">Acuan Pembayaran</label>
