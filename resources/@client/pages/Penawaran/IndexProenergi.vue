...
/* Actions */
function openCreate() {
  router.push({ name: 'penawarans-create-proenergi' })
}

function openDetail(id: number) {
  router.push({ name: 'penawarans-detail-proenergi', params: { id } })
}

function openEdit(id: number) {
  router.push({ name: 'penawarans-edit-proenergi', params: { id } })
}
...
async function submitDelete() {
  ...
  try {
    await axios.delete(`/api/penawarans-proenergi/${deleteTarget.value.id}`)
...
<template>
  <div class="page-content-wrapper">
    <div class="intro-y flex flex-col gap-4">
      <PageHeader title="Penawaran Proenergi" description="Kelola data penawaran ke customer (Proenergi)">
        <template #action>
