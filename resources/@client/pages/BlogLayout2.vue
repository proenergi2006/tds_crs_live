...
<template>
  ...
  <div class="grid grid-cols-12 gap-6 mt-5 intro-y">
    <!-- BEGIN: Blog Layout -->
    <div
      v-for="(faker, fakerKey) in _.take(fakerData, 6)"
      :key="fakerKey"
      class="col-span-12 intro-y md:col-span-6 box"
    >
      <div
      ...
      >
        <img
          ...
          :src="faker.images[0]"
        />
        <div class="absolute z-10 flex items-center w-full px-5 pt-6">
          ...
          <Menu class="ml-3">
            ...
            >
              <Lucide icon="MoreVertical" class="w-4 h-4 text-white" />
            </Menu.Button>
            <Menu.Items class="w-40">
              <Menu.Item>
        ...
        <div class="absolute bottom-0 z-10 px-5 pb-6 text-white">
          ...
          <a href="" class="block mt-3 text-xl font-medium">
            {{ faker.news[0].title }}
          </a>
        </div>
      </div>
