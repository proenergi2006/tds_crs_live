...
const api = createResourceApi('/review/bm/customer-verifications')
...
async function fetchData(page = 1) {
  ...
  try {
    ...
    error('Gagal', e.response?.data?.message ?? 'Gagal memuat antrean BM.')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="page-content-wrapper">
    <div class="intro-y flex flex-col gap-4">
      <PageHeader title="Verifikasi Data Customer — BM"
        :description="`Keputusan final persetujuan data customer. Antrean saat ini: ${queueCount}.`" />
      ...
      <DataList v-model:search="searchQuery" v-model:per-page="perPage" :loading="loading" :empty="rows.length === 0"
        ...
        loading-text="Memuat antrean BM..." empty-description="Tidak ada verifikasi yang menunggu keputusan BM."
          ...
          <Table.Tr v-for="(row, idx) in rows" :key="row.id_verification" class="transition hover:bg-slate-50">
            ...
            <Table.Td class="text-center">
              <div class="inline-flex items-center justify-center gap-1">
                <RouterLink :to="{ name: 'bm-customer-detail', params: { id: row.id_verification } }"
