...
const updatedByInfo = computed(() => {
  if (props.form.updated_by === null || props.form.updated_by === '') {
    return `Created By ${props.form.created_by || 'N/A'}`;
  } else {
    return `Last Updated By ${props.form.updated_by || 'N/A'}`;
  }
});
</script>
...
<template>
  <FormModal :open="open" :title="modalTitle" :description="modalDescription" :loading="loading" :error="error"
    ...
    <div class="space-y-3">
      <!-- Nama Jenis Produk (required) -->
      <div>
        <FormLabel htmlFor="edit-nama">Nama Jenis Produk <span class="text-danger">*</span></FormLabel>
        <FormInput id="edit-nama" v-model="form.nama" placeholder="Nama Jenis Produk"
          :class="fieldErrors.nama ? 'border-rose-500' : ''" required />
        <small v-if="fieldErrors.nama" class="text-rose-600">{{ fieldErrors.nama
        }}</small>
      </div>
