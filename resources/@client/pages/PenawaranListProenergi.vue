<template>
  <div class="p-6 intro-y">
    ...
    <div class="flex items-center mb-4">
      <h2 class="text-lg font-medium">Daftar Penawaran Agen Proenergi</h2>

      <div class="ml-auto flex gap-2">
        <RouterLink :to="{ name: 'penawarans-create-proenergi' }">
...
<div class="flex flex-col items-center gap-1">
  ...
  <span
    :class="[
      ...
      disposisiBadgeClass(String(pen.disposisi_penawaran))
    ]"
  >
  ...
  >
    {{ getDisposisiTanggal(pen) }}
            ...
            <Table.Td class="px-4 py-3 whitespace-nowrap text-center flex justify-center">
              <!-- Detail -->
              <RouterLink
                :to="{ name: 'penawarans-detail-proenergi', params: { id: pen.id_penawaran } }"
              ...
              <RouterLink
                  :to="{ name: 'penawarans-edit-proenergi', params: { id: pen.id_penawaran } }"
...
async function fetchData(page = 1) {
  try {
    const res = await axios.get('/api/penawarans-proenergi', {
      params: {
        page,
...
function disposisiBadgeClass(v: string) {
switch (v) {
  case '1': // Draft
...
// 3 = menunggu verifikasi OM, biasanya BM sudah approve
...
// opsional: kalau mau draft / menunggu BM tidak tampil tanggal
return ''
}
...
function confirmDelete(id: number, nomor: string) {
  Swal.fire({
    ...
    try {
      await axios.delete(`/api/penawarans-proenergi/${id}`)
