<template>
    <div class="p-6 intro-y">
      <div class="flex items-center mb-4">
        <h2 class="text-lg font-medium">OA Truck</h2>
        <RouterLink :to="{ name: 'oa-trucks-create' }" class="ml-auto">
          <Button variant="primary">Tambah OA Truck</Button>
        </RouterLink>
      </div>
  ...
  <RouterLink :to="{ name: 'oa-trucks-edit', params: { id: item.id } }" class="text-blue-600 hover:text-blue-800">
  ...
  async function fetchData(page = 1) {
    try {
      const { data } = await axios.get('/api/ongkos-trucks', {
      ...
      Swal.fire('Error', 'Gagal memuat data OA Truck', 'error')
    }
  }
  ...
  function confirmDelete(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axios.delete(`/api/ongkos-trucks/${id}`)
