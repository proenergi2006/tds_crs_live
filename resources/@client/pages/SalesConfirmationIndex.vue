<template>
    <div class="p-6 intro-y">
      <div class="flex items-center mb-4">
        <h2 class="text-lg font-semibold">Sales Confirmation</h2>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-2 mb-4 intro-y sm:flex-nowrap">
        ...
        <Button class="!box" @click="fetchData(1)">
          Cari
        </Button>
            ...
            <Table.Tr v-for="(row, i) in rows" :key="row.id_poc" class="hover:bg-slate-50">
              ...
              <Table.Td class="whitespace-nowrap">
                <span class="font-semibold">
                  {{ kode(row.customer?.kode_pelanggan) }}
                </span>
                &nbsp;-&nbsp;
              ...
              <Table.Td class="text-center">
                <button class="text-sky-600 hover:text-sky-800" @click="$router.push({name:'sales-confirmations-detail', params:{id: row.id_poc}})">
  <Lucide icon="Info" class="w-5 h-5"/>
            ...
            <Table.Tr v-if="!rows.length">
              <Table.Td colspan="7" class="text-center py-6 text-slate-500">
  ...
  function fmtCurrency(n: number | string) {
    const num = Number(n ?? 0)
    return isNaN(num) ? 'Rp 0' : `Rp ${num.toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  }
  function kode(k?: string|null) {
    return k ? `E-${k}`.replace(/^E-?/, 'E-') : '-'
  }
  function badgeClass(v: number | string) {
    const s = String(v)
    return {
      'text-slate-600': s === '0',
      'text-emerald-600': s === '1', // Terverifikasi Admin
  ...
  async function fetchData(toPage = 1) {
    try {
      const { data } = await axios.get('/api/sales-confirmations', {
        params: { page: toPage, per_page: perPage.value, search: q.value || undefined }
      })
  ...
  function openDetail(id: number) {
    // arahkan ke detail yang sudah ada (kalau komponen detail-mu sudah siap)
    // router.push({ name: 'po-customers-detail', params: { id } })
    Swal.fire({ title: 'Detail', text: `POC ID: ${id}`, icon: 'info' })
  }
