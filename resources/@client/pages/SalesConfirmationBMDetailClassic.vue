  ...
  <div class="p-6 space-y-4">
    ...
    <div class="border rounded bg-white">
        ...
        <table class="w-full text-sm">
          <tbody>
            ...
            <tr><td class="p-3">TOP Customer</td>
              <td class="p-3">
                : {{ h.poc?.tipe_bayar
                      ? (h.poc.tipe_bayar === 'CREDIT' && h.poc.termin_hari
                          ? `${h.poc.tipe_bayar_label} — ${h.poc.termin_hari} Hari`
                          : (h.poc.tipe_bayar_label ?? h.poc.tipe_bayar))
                      : '-' }}
              </td></tr>
            ...
            <tr><td class="p-3">Volume Penawaran</td>
              <td class="p-3">: {{ formatNumber(h.poc?.volume_poc) }} m³</td></tr>
            ...
            <tr><td class="p-3">Tanggal PO</td>
              <td class="p-3">: {{ formatDate(h.poc?.tanggal_poc) }}</td></tr>

            <tr><td class="p-3">Tgl Pengiriman</td>
              <td class="p-3">: {{ formatDate(h.poc?.supply_date) }}</td></tr>
      ...
      <div class="p-3 border-t flex gap-2">
        <Button variant="outline-secondary" @click="router.back()">Kembali</Button>
        <Button variant="warning" @click="editNomorPo">Edit Nomor PO</Button>
        <Button variant="success" @click="gotoPlan" :disabled="h.sc?.disposisi !== 2">Jadwal Kirim</Button>
        <Button variant="success" @click="closePo">Close PO</Button>
      </div>
    </div>
  </div>
</template>
...
const produkLabel = computed(() => h.value?.poc?.produk_poc || h.value?.penawaran?.produk_name || '-')

function dispTime(d?: string): string {
  if (!d) return '-'
...
function badgeClass(v?: number | string): Record<string, boolean> {
  const s = String(v ?? '')
  return {
    'text-emerald-600': s === '2',
    'text-slate-600': s === '1' || s === '0',
    'text-orange-600': s === '3',
...
async function editNomorPo(): Promise<void> {
...
async function closePo(): Promise<void> {
