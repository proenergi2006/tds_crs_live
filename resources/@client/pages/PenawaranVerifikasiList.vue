<template>
  <div class="p-6 intro-y">
    <div class="flex items-center mb-4">
      <h2 class="text-lg font-medium">Daftar Verifikasi Penawaran 1</h2>
    </div>
          ...
          >
            ...
            <Table.Td class="px-4 py-3 text-center whitespace-nowrap">
              <span
                ...
                :class="{
                  'text-slate-600': pen.status === 'draft',
                  'text-amber-600': pen.status === 'menunggu',
                  'text-emerald-700': pen.status === 'approved bm' || pen.status === 'approved om',
                  'text-rose-600': pen.status === 'rejected bm' || pen.status === 'rejected om'
                }"
              >
                {{ pen.status }}
              </span>
...
/** Badge class untuk Disposisi.
 * Khusus '2' (Menunggu Verifikasi BM) -> ABU-ABU (bg-gray-200 text-gray-700)
 */
function disposisiBadgeClass(value: string | number): string {
  switch (String(value)) {
    case '1': return 'bg-slate-100 text-slate-700'
    case '2': return 'bg-gray-200 text-gray-700'        // << abu-abu
