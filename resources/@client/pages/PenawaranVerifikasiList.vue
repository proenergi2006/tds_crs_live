    ...
    <div class="flex flex-col gap-4 mb-6 lg:flex-row lg:items-start lg:justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500">
          Monitor penawaran yang menunggu verifikasi Branch Manager.
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 w-full lg:w-auto">
        ...
        <div class="rounded-2xl border border-gray-200 bg-gray-50 px-5 py-4 shadow-sm min-w-[180px]">
          <div class="text-xs font-semibold tracking-wide uppercase text-gray-500">Menunggu BM</div>
          <div class="mt-2 text-3xl font-bold text-gray-700">{{ totalMenungguBm }}</div>
        </div>

        <div class="rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 shadow-sm min-w-[180px]">
          <div class="text-xs font-semibold tracking-wide uppercase text-rose-500">Ditolak BM</div>
          <div class="mt-2 text-3xl font-bold text-rose-600">{{ totalDitolakBm }}</div>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <Table class="min-w-full">
          <Table.Thead class="bg-slate-50">
            <Table.Tr>
              <Table.Th class="whitespace-nowrap text-center">No</Table.Th>
              <Table.Th class="whitespace-nowrap">Nomor Penawaran</Table.Th>
              <Table.Th class="whitespace-nowrap">Customer</Table.Th>
              <Table.Th class="whitespace-nowrap">Cabang</Table.Th>
              <Table.Th class="whitespace-nowrap text-center">Masa Berlaku</Table.Th>
              <Table.Th class="whitespace-nowrap text-center">Status</Table.Th>
              <Table.Th class="whitespace-nowrap text-center">Disposisi</Table.Th>
              <Table.Th class="whitespace-nowrap text-center">Aksi</Table.Th>
            </Table.Tr>
          </Table.Thead>
          ...
          <Table.Tbody>
            ...
            <Table.Tr v-else-if="!penawarans.length">
              <Table.Td colspan="8" class="py-14 text-center">
                <div class="flex flex-col items-center gap-3 text-slate-500">
                  ...
                  <div class="text-sm">Belum ada penawaran yang sesuai dengan filter pencarian.</div>
                </div>
              </Table.Td>
            </Table.Tr>
            ...
            >
              ...
              <Table.Td class="text-center">
                <RouterLink
                  :to="{ name: 'penawarans-verifikasi-detail', params: { id: pen.id_penawaran } }"
...
async function fetchData(page = 1) {
  loading.value = true
  try {
    const res = await axios.get('/api/penawarans/bm', {
      params: {
        page,
...
const totalMenungguBm = computed(() =>
  penawarans.value.filter((item: any) => String(item.disposisi_penawaran) === '2').length
)

const totalDitolakBm = computed(() =>
  penawarans.value.filter((item: any) => String(item.disposisi_penawaran) === '5').length
)
