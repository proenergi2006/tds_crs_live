<template>
    <div class="min-h-screen bg-slate-50">
      <div class="bg-green-700 text-white">
        <div class="max-w-7xl mx-auto px-5 py-4">
          <div class="text-2xl font-semibold">Verifikasi Data Customer — OM</div>
          <div class="text-white/80 text-sm">
            Menampilkan data dengan status: is_reviewed = 1 & disposisi = OM
          </div>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-5 py-6 space-y-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="ml-auto text-sm text-slate-600">
            Antrean OM: <span class="font-semibold">{{ stats.queue }}</span>
          </div>
        </div>
          ...
          <table class="min-w-full text-sm">
            ...
            <tbody>
              ...
              <tr v-for="(r, i) in rows" :key="r.id_verification" class="border-t">
                ...
                <td class="px-4 py-2">
                  <RouterLink
                    :to="{ name: 'om-customer-detail', params: { id: r.id_verification } }"
  ...
  async function fetchStats(){
    const { data } = await axios.get('/api/review/om/customer-verifications/stats')
    stats.value = data
  }
  async function fetchList(p=1) {
    ...
    try {
      page.value = p
      const { data } = await axios.get('/api/review/om/customer-verifications', {
