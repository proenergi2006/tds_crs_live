<template>
  ...
  async function preview() {
    try {
      const response = await axios.get(`/api/penawarans-proenergi/${id}/preview`, { responseType: 'blob' })
  ...
  async function previewLang(lang: 'id' | 'en') {
    try {
      const response = await axios.get(`/api/penawarans-proenergi/${id}/preview`, { params: { lang }, responseType: 'blob' })
      const blob = new Blob([response.data], { type: 'application/pdf' })
      const pdfUrl = URL.createObjectURL(blob)
      window.open(pdfUrl, '_blank')
      setTimeout(() => URL.revokeObjectURL(pdfUrl), 60000)
