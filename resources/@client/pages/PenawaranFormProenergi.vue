<template>
            ...
            <div>
              <label class="block text-sm font-medium mb-1">Sampai Dengan</label>
               <input
                ...
                :class="inputClass('sampai_dengan')"
                readonly
              />
            </div>
          </div>
  ...
  // ambil akhir bulan dari tanggal yang dipilih
  ...
  try {
    ...
    console.log('CHECK HARGA:', data);

    if (data.found) {
      item.harga_price_list = Number(
        data.harga_price_list_pe ?? data.harga_price_list ?? 0
      );
    ...
    payloadOngkos.push({
     ...
     console.group('DEBUG ONGKOS')
      ...
      const payload = {
        ...
        top_hari: form.tipe_pembayaran === 'TOP'
    ? parseInt(String(form.top_hari || '0'), 10) || null
    : null,
